 ...
 * 
 * Name: Kaifu Wang		AndrewID: kaifuw
 */
#include <assert.h>
#include <stdio.h>
#include <stdlib.h>
...
/*my helper functions*/
void redir(struct cmdline_tokens *tok);
...
eval(char *cmdline) 
{
	...
	switch (tok.builtins) {
		...
		default:
			...
			//block signals receiving in parent
			sigprocmask(SIG_BLOCK, &mask, &mask2); 

			if ((pid = fork()) == 0){
				//unblock signals receiving in child
				sigprocmask(SIG_UNBLOCK, &mask, NULL);
				setpgid(0, 0);
				//handle redirection
				redir(&tok);
				execvp(tok.argv[0], tok.argv);
...
void redir(struct cmdline_tokens *tok) {
	int fd1, fd2;
	if (tok->infile != NULL) {
		fd1 = open(tok->infile, O_RDONLY);
		dup2(fd1, STDIN_FILENO);
		close(fd1);
	}
	if (tok->outfile != NULL) {
		fd2 = open(tok->outfile, O_WRONLY);
...
void builtin_BG(struct cmdline_tokens *tok) {		
	...
	//parsing pid
	if (tok->argv[1][0] == '%') {
		if (!atoi(tok->argv[1]+1)){
			pid = 0;
			app_error("Job id not found\n");
		}
		else 
	...
	kill(-pid, SIGCONT);
	printf("[%d] (%d) %s\n", pid2jid(pid), pid, getjobpid(job_list, pid)->cmdline);
